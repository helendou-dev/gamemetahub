...
export default function ReleasesPage() {
  ...
  const items = allArticles.filter((a) => a.type === 'game_release');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 md:py-16">
      <div className="mb-10">
        ...
        <p style={{ color: 'var(--text-secondary)' }}>
          {items.length} launch guides and preparation checklists
        </p>
      </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="font-semibold text-lg" style={{ color: 'var(--text-primary)' }}>No release guides yet</p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((item) => (
            <Link key={`${item.game}/${item.slug}`} href={item.url} className="glow-card block group">
              ...
              <div className="p-5">
                {!item.image && <span className="type-badge type-badge-release">🚀 Launch Guide</span>}
